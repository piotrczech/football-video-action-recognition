...
def render() -> None:
    ...
    if not analyses:
        st.info("No saved match analyses found in `outputs/videos`.")
        return
...
def _show_analysis(analysis: SavedMatchAnalysis) -> None:
    ...
    if preview_path.exists():
        ...
        st.warning("Video preview (WebM) is no longer available on disk.")
    ...
    with actions[0]:
        if download_path is not None and download_path.exists():
            ...
            st.caption("MP4 download file is not available.")
    with actions[1]:
        st.caption(f"Analysis ID: `{analysis.analysis_id}`")
        st.caption(f"Preview: `{preview_path}`")
        if download_path is not None:
            st.caption(f"Download: `{download_path}`")

    if analysis.summary is None:
        st.info("Metadata for this analysis is not available.")
        return
...
def _show_summary(summary: dict[str, Any]) -> None:
    ...
    metrics[0].metric("Clip duration", _duration_value(metadata.get("duration_seconds")))
    metrics[1].metric("Sampling", _fps_value(summary.get("sample_fps")))
    metrics[2].metric("Analyzed frames", _count_value(summary.get("sampled_frames")))
    metrics[3].metric("Detections", _count_value(stats.get("total_detections")))
    metrics[4].metric("Tracks", _count_value(stats.get("track_count")))
...
def _analysis_row(analysis: SavedMatchAnalysis) -> dict[str, str]:
    ...
    stats = _mapping(summary.get("stats"))
    return {
        "Video": analysis.preview_path.name,
        "Saved": _modified_at_value(analysis.modified_at_ns),
        "Model": _string_value(summary.get("model")),
        "Run": _string_value(summary.get("resolved_run_name")),
        "Duration": _duration_value(metadata.get("duration_seconds")),
        "Frames": _count_value(summary.get("sampled_frames")),
        "Detections": _count_value(stats.get("total_detections")),
    }
